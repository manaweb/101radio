...
class Album extends CI_Controller {
	...
	function index() {
		...
		$data['itens'] = array(
			array("nome" => 'Album', 'icone' => 'fa fa-picture-o', 'url' => "painel/album"),
		);
		...
		$acoes = array(1,2,3);
		$data['lista'] = $this->Toolmodel->painelListar($campos,$query,$acoes,'album/acoes','id',true);
	...
	function cadastrar() {
	    ...
	    $data['itens'] = array(
			array("nome" => 'Album', 'icone' => 'fa fa-picture-o', 'url' => "painel/album"),
			array("nome" => 'Novo album', 'icone' => 'fa fa-plus-square', 'url' => "painel/album/adicionar"),
		);
	...
	function acoes($action='',$id=0) {
		switch($action) {
	    	...
	    	case 'insert':
				...
				$caminho = "";
				$realpath = FCPATH.'assets'.DIRECTORY_SEPARATOR.'img'.DIRECTORY_SEPARATOR.md5($nome);
			...
			case 'update':
				...
				$this->Toolmodel->alterar('album', $dadosUpdate); 
				redirect("painel/album");
			break;
		
			case 'delete':
				$this->Toolmodel->excluir('album', $id,'assets'.DIRECTORY_SEPARATOR.'img'.DIRECTORY_SEPARATOR.'avatar'.DIRECTORY_SEPARATOR.'','avatar');
				redirect("painel/album");
			break;
		}
	}
